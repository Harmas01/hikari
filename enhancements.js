...
(() => {
  if (!document.querySelector('.mobile-nav')) {
    document.body.insertAdjacentHTML('beforeend', `<nav class="mobile-nav" aria-label="Мобильная навигация"><button type="button" data-view="home" class="active"><i>⌂</i><span>Главная</span></button><button type="button" data-view="catalog"><i>▦</i><span>Каталог</span></button><button type="button" data-mobile-section="newRow"><i>✦</i><span>Новинки</span></button><button type="button" data-mobile-profile><i>◉</i><span>Профиль</span></button></nav>`);
  }
  ...
  mobileNav?.addEventListener('click', (event) => {
    ...
    if (button.dataset.mobileSection) {
      if (typeof setView === 'function') setView('home');
      setTimeout(() => document.querySelector(`#${button.dataset.mobileSection}`)?.closest('.poster-row')?.previousElementSibling?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 100);
    }
    if (button.hasAttribute('data-mobile-profile')) document.querySelector('.topbar .profile')?.click();
  });

  document.addEventListener('click', (event) => {
    const viewTarget = event.target.closest('[data-view]')?.dataset.view;
    if (!viewTarget || !mobileNav) return;
    mobileNav.querySelectorAll('button').forEach((button) => button.classList.toggle('active', button.dataset.view === viewTarget));
  });

  const style = document.createElement('style');
  style.textContent = `
    html,body{max-width:100%;overflow-x:hidden}.mobile-nav{display:none}
    @media(max-width:720px){
      :root{--mobile-pad:14px}body{padding-bottom:72px;-webkit-tap-highlight-color:transparent}.app-shell{min-width:0}.topbar{position:sticky;top:0;z-index:25;height:58px;padding:0 var(--mobile-pad);gap:10px;background:rgba(13,14,17,.9);backdrop-filter:blur(18px);border-bottom:1px solid #ffffff0d}.brand{font-size:12px;letter-spacing:.12em}.brand span{width:29px;height:29px;border-radius:9px;font-size:13px}.header-actions{margin-left:auto;gap:7px}.icon-button,.profile{width:38px;height:38px;min-width:38px}.main-nav{display:none!important}
      .mobile-nav{position:fixed;z-index:40;left:8px;right:8px;bottom:calc(7px + env(safe-area-inset-bottom));display:grid;grid-template-columns:repeat(4,1fr);height:58px;padding:5px;background:rgba(22,23,28,.94);border:1px solid #363842;border-radius:16px;box-shadow:0 12px 36px #000a;backdrop-filter:blur(20px)}.mobile-nav button{display:grid;place-items:center;align-content:center;gap:2px;min-width:0;border:0;border-radius:11px;background:transparent;color:#81838d;font:500 9px Inter;cursor:pointer}.mobile-nav button i{font-style:normal;font-size:17px;line-height:1}.mobile-nav button.active{background:#8563ef1d;color:#cfc3ff}.mobile-nav button.active i{color:#9d7cff}
      .hero{height:auto;min-height:560px}.hero-backdrop{background-position:62% center!important}.hero-content{padding:88px var(--mobile-pad) 112px;max-width:none}.hero h1{max-width:92%;font-size:clamp(31px,10vw,44px);line-height:1.04}.hero-copy{max-width:92%;font-size:13px;line-height:1.6}.hero .meta{gap:8px;flex-wrap:wrap}.hero-actions{display:grid;grid-template-columns:1fr 1fr;gap:9px;width:100%}.hero-actions button{min-height:46px;padding:0 12px}.hero-progress{display:none!important}
      .content,.content:not(.watch-layout){padding-left:var(--mobile-pad);padding-right:var(--mobile-pad)}.section-head{margin-top:28px;margin-bottom:12px}.section-head h2{font-size:18px}.section-head a{font-size:10px}.poster-row{display:flex!important;grid-template-columns:none!important;gap:11px;overflow-x:auto;scroll-snap-type:x proximity;scrollbar-width:none;margin-right:calc(var(--mobile-pad) * -1);padding-right:var(--mobile-pad);overscroll-behavior-inline:contain}.poster-row::-webkit-scrollbar{display:none}.poster-row .card{flex:0 0 142px;scroll-snap-align:start}.card h3{font-size:11px;line-height:1.35}.card p{font-size:9px}.poster{border-radius:11px}.continue-row{grid-template-columns:1fr!important}.continue-card{min-height:94px;height:auto;padding-right:11px}.continue-card img{width:76px}.continue-card h3{font-size:11px}.continue-card p{font-size:9px}
      .watch-layout{display:block;padding:10px var(--mobile-pad) 24px}.back-link{min-height:40px}.player{width:calc(100% + var(--mobile-pad) * 2);margin-left:calc(var(--mobile-pad) * -1);border-radius:0;aspect-ratio:16/9}.player-coming-soon strong{font-size:21px}.player-coming-soon small{font-size:10px}.player-controls{padding:9px 10px;gap:6px}.player-controls button{min-width:30px;min-height:30px}.player-controls .control-label,.player-controls button:nth-last-child(2),.control-time{display:none!important}.timeline{min-width:40px}.episode-switch{min-height:50px;padding:0 4px}.episode-switch button{min-height:44px;padding:0 8px}.episode-switch span{display:none}.episode-switch strong{font-size:11px}.watch-title{display:grid;grid-template-columns:1fr;gap:13px;padding:17px 0}.watch-title h2{font-size:20px}.watch-title .favorite{width:100%;min-height:44px}.episodes{margin-top:8px;max-height:none;border-radius:12px}.episode{min-height:58px}.comments{padding:17px 12px}.comment-list{max-height:360px}.comment-form input{min-height:44px;font-size:16px}.comment-form button{min-width:44px}.comment-actions button{min-height:34px}
      #anime .anime-page[data-redesigned]{display:grid;grid-template-columns:1fr!important;gap:20px;padding:18px var(--mobile-pad) 4px}.anime-page[data-redesigned]::before{height:180px}.anime-sidebar{display:grid;grid-template-columns:118px minmax(0,1fr);gap:12px}.anime-sidebar .anime-cover{grid-row:1/3;width:118px;max-width:none;aspect-ratio:2/3}.anime-sidebar .hero-actions{grid-column:2;display:flex;flex-direction:column}.anime-sidebar .hero-actions button{min-height:44px}.anime-facts{grid-column:1/-1;grid-template-columns:1fr 1fr;padding:7px 12px}.anime-facts div{display:grid;gap:3px;padding:9px 4px}.anime-facts b{text-align:left;font-size:10px}.anime-main-panel{padding-top:0}.anime-title-score{display:block;min-height:0}.anime-title-score>h1{font-size:29px;margin-top:26px}.anime-title-score>.alternative{font-size:12px}.community-score{text-align:left;padding:12px 0}.anime-tabs{gap:22px;margin-bottom:18px;overflow-x:auto;scrollbar-width:none}.anime-tabs button{flex:0 0 auto;min-height:44px}.anime-page[data-redesigned] .description{font-size:12px}.anime-related-card{padding:13px}.anime-page[data-redesigned] .anime-awards{display:grid!important;grid-template-columns:1fr!important}.rating-overview{padding:16px}.rating-heading{align-items:flex-start}.rating-bars{grid-template-columns:1fr!important}.anime-detail-redesign{padding:12px var(--mobile-pad) 35px!important}.anime-detail-redesign .episode-grid{grid-template-columns:1fr!important}.episode-grid button{min-height:56px}.community-editor{padding:13px}.community-editor input,.community-editor textarea{font-size:16px}.community-editor>div{gap:8px}.community-editor button{min-height:42px}
      .catalog{padding-top:28px}.catalog-intro h1{font-size:31px}.catalog-toolbar[data-functional]{display:block}.catalog-search{width:100%;min-height:44px}.catalog-search input{font-size:16px}.catalog-filter-controls{display:grid!important;grid-template-columns:1fr 1fr!important;gap:8px!important;overflow:visible!important;margin-top:12px}.catalog-select,.catalog-sort{min-width:0!important;grid-column:auto!important}.catalog-select select{height:44px;font-size:11px}.catalog-grid{grid-template-columns:repeat(2,minmax(0,1fr))!important;gap:12px 10px}.catalog-grid .card{min-width:0}.catalog-results-summary{margin-top:12px}
      .profile-view{padding:18px var(--mobile-pad) 35px}.profile-page-card{display:grid;grid-template-columns:1fr!important;gap:18px!important;padding:19px!important;min-height:0}.profile-side{display:flex!important;padding:0 0 15px!important;border-right:0!important;border-bottom:1px solid #30313a!important}.profile-side .profile-page-avatar{width:82px!important;height:82px!important}.profile-side-nav{grid-template-columns:repeat(3,1fr)!important}.profile-side-nav button{min-height:42px;text-align:center!important;padding:7px 3px!important}.profile-page-main,.profile-activity{grid-column:1!important;max-width:none}.profile-page-main h1,.profile-activity h1{font-size:25px}.profile-stats{grid-template-columns:repeat(3,1fr)}.profile-stats div{padding:10px 5px!important}.profile-page-main input,.profile-page-main textarea{font-size:16px!important}.profile-actions{display:grid}.profile-actions button{min-height:44px}.watched-item{min-height:70px}.theme-options,.frame-options{display:grid;grid-template-columns:1fr 1fr}
      .search-overlay{padding:68px var(--mobile-pad) 20px;align-items:start}.search-box,.quick-results{width:100%}.search-box{padding:8px 11px}.search-box input{font-size:16px}.search-box kbd{display:none}.quick-result{min-height:68px}.auth-modal{padding:10px}.auth-card{max-height:calc(100dvh - 20px);overflow:auto;padding:20px;border-radius:14px}.auth-card input{font-size:16px;min-height:44px}.auth-card button{min-height:42px}
      button,a,input,select,textarea{touch-action:manipulation}.toast{left:12px;right:12px;bottom:82px;text-align:center;transform:translateY(140px)}.toast.show{transform:translateY(0)}
    }
    @media(max-width:380px){.catalog-filter-controls{grid-template-columns:1fr!important}.anime-sidebar{grid-template-columns:104px minmax(0,1fr)}.anime-sidebar .anime-cover{width:104px}.hero h1{font-size:30px}.poster-row .card{flex-basis:132px}.profile-stats b{font-size:14px!important}}
    @media(max-width:720px) and (orientation:landscape){.player{width:100%;margin-left:0;border-radius:12px}.mobile-nav{height:52px}.hero{min-height:470px}}
  `;
  document.head.append(style);
})();